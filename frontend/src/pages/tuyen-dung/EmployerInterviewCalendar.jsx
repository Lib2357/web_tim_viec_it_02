...
export default function EmployerInterviewCalendar() {
  ...
  const [status, setStatus] = useState('Tat ca trang thai')
  const [role, setRole] = useState('Moi vi tri')
  const [view, setView] = useState('Tuan')
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setView((interview.viewModes || [])[1] || 'Tuan')
      setStatus((interview.statusOptions || [])[0] || 'Tat ca trang thai')
      setRole((interview.roleOptions || [])[0] || 'Moi vi tri')
    }
    loadData().catch(() => {})
  }, [])
  ...
  const filtered = useMemo(() => {
    return items.filter((item) => {
      const statusOk = status === 'Tat ca trang thai' || item.type === status
      const roleOk = role === 'Moi vi tri' || item.role === role
  ...
  return (
    <div className="bg-[#f7f9fc] text-on-surface">
      ...
      <main className="ml-64 min-h-screen p-5">
        <h1 className="mb-4 text-[24px] font-bold">Employer Interview Calendar</h1>
