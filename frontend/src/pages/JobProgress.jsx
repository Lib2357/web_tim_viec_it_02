...
export default function JobProgress() {
  ...
  const [statusOptions, setStatusOptions] = useState(['Tat ca'])
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        const options = mock?.jobProgress?.statusOptions || ['Tat ca']
        setStatusOptions(options)
        setStatus(options[0] || 'Tat ca')
  ...
  const filteredJobs = useMemo(() => {
    ...
    return jobs.filter((item) => {
      const statusOk = status === 'Tat ca' || item.status === status
  ...
  return (
    <div className="bg-[#f7f9fc] text-on-surface">
      ...
      <main className="ml-64 min-h-screen p-5">
        <h1 className="mb-4 text-[22px] font-semibold text-slate-900">Tien do cong viec</h1>
        <section className="mb-4 grid grid-cols-1 gap-3 lg:grid-cols-12">
          <input
            ...
            placeholder="Tim theo ma job, du an, khach hang..."
            value={search}
