...
export default function JobList() {
  ...
  const [status, setStatus] = useState('Chon trang thai')
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      setStatus(options[0] || 'Chon trang thai')
  ...
  const stats = useMemo(() => {
    ...
    const pending = appliedJobs.filter((j) => j.status === 'Cho duyet').length
    const accepted = appliedJobs.filter((j) => j.status === 'Chap nhan').length
  ...
  return (
    <div className="bg-[#f7f9fc] text-on-surface">
      ...
      <main className="ml-64 min-h-screen p-5">
        <h1 className="mb-4 text-[22px] font-bold">Job da ung tuyen</h1>
